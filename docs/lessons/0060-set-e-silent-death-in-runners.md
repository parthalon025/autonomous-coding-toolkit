...
fix: "Add trap '_log_exit $?' EXIT for diagnostics, trap '' HUP for background survival, and wrap non-critical commands in { ... } || warn blocks"
example:
  bad: |
    set -euo pipefail
    for batch in ...; do
...
2. **No signal handling.** Background processes need `trap '' HUP` to survive terminal disconnects. Without it, session management events (context compaction, terminal hangup) send SIGHUP which kills the process tree.
